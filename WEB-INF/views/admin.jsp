...
<head>
    ...
    .btn-parcel {
        background-color: rgb(148, 2, 148);
    }
...
const STATUS_OPTIONS = ["In transit", "Dispatched", "Shipped", "Delivered", "Cancelled", "Others"];

STATUS_OPTIONS.forEach(status => {
    const option = document.createElement('option');
    option.value = status;
    option.text = status;
    if (status === parcel.status) option.selected = true;
    select.appendChild(option);
});

select.disabled = true; // Initially disabled
select.onchange = () => updateStatus(parcel.id, select.value);
...
editBtn.onclick = () => select.disabled = false;
...
//tr.appendChild(tdStatus); // ✅ Append status cell
    ...
    try {
        const response = await fetch(`/api/admin/parcels/${parcelId}/status`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
    </div>
    ...
    <div id="parcelManagementSection" style="display: none;">
        ...
        <button id="addParcelBtn" onclick="openParcelPopup()" class="btn btn-parcel" style="margin-bottom: 20px;">
            Add Parcel
        </button>
